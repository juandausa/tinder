 ...
 * @constructor
 */
function InterestsController($scope, $state, $mdDialog, $mdMedia, InterestsService, HeaderService, ToastService) {
    var self = this;
    self.data = {
        interests: []
      };
    self.initialize = initialize;
    self.newInterest = newInterest;
    ...
    function initialize() {
        HeaderService.setTitle("Interests");
        HeaderService.showProgressBar();
        
        InterestsService.getInterests().then(
            function succes(response) {
                // self.data.interests = response.interests;
                self.data.interests = response.interests.map(function(int,index) {
                    return buildInterest(int,index);
                });
                HeaderService.hideProgressBar();
                ToastService.showSuccess('Interests: ' + self.data.interests.length);
            },
    ...
    function newInterest(ev) {
        ...
        .then(function(response) {
            self.data.interests.push(buildInterest(response,self.data.interests.length))
        }, function(response) {
    ...
    function buildInterest(it,index){
            it.background  = "";
            ...
            it.background = COLORS[getRandomInt(0,COLORS.length)];
            if (it.value.length > 8 || it.category.length > 7){
                it.span.col++;
            }
          return it;
        }
    ...
    $scope.accept = function() {
        InterestsService.createInterest($scope.interest)
        .then(
            function successCallback(response) {
                $mdDialog.hide($scope.interest);
              }, 
