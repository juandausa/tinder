...
public:
    ...
    EasyCurl(const EasyCurl& rhs) { //copy ctor for duplicating the handle so you can close the otherone
    ...
    bool UseCookieEngine() {
        return curl_easy_setopt(m_curl,CURLOPT_COOKIEFILE,"Q:\\asdfiw48rjasd-93jasj9f842rf90sdj.98r.34.as4.") == CURLE_OK;
...
//		error = new char[CURL_ERROR_SIZE];
    ...
    void CloseFile() { if(m_file) fclose(m_file); }
    ...
    static curl_slist  * SlistAppend(struct  curl_slist *list, const char * string) { return curl_slist_append(list,string); }
