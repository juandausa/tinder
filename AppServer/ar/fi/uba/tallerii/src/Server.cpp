...
void Server :: ev_handler(struct mg_connection *nc, int ev, void *ev_data) {
    ...
    switch (ev) {
        case MG_EV_HTTP_REQUEST:
        {
            ...
            } else if ((mg_vcmp(&hm->uri, "/candidates") == 0) && mg_vcmp(&hm->method, "GET") == 0) {
                UserController user_controller(user_service);
                user_controller.handle_show_candidates(nc, hm, response);
            ...
            } else if ((mg_vcmp(&hm->uri, "/matches") == 0) && mg_vcmp(&hm->method, "GET") == 0) {
                UserController user_controller(user_service);
                user_controller.handle_get_matches(nc, hm, response);
